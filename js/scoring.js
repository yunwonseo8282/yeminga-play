/**
 * scoring.js — MBUY 소비 유형 테스트 채점 모듈
 *
 * 이 파일은 순수 계산 함수만 담습니다.
 * 유형 매칭 / 유사도 / 페이지 이동은 Day 10 Step 2~5 에서 추가 예정.
 *
 * 의존성:
 *   window.QUESTIONS_SPENDING_TYPE (questions-spending-type.js 에서 로드)
 */
...
(function () {
  ...
  function calculateDimensionScores(answers) {
    ...
    if (!questions || questions.length !== 30) {
      console.error('[scoring] QUESTIONS_SPENDING_TYPE 이 로드되지 않았습니다.');
      return null;
    }
    if (!answers || answers.length !== 30) {
      console.error('[scoring] answers 배열 길이가 30이 아닙니다:', answers && answers.length);
      return null;
    }
  ...
  function classifyLMH(rawScores) {
    if (!rawScores || rawScores.length !== 10) {
      console.error('[scoring] rawScores 길이가 10이 아닙니다.');
      return null;
    }
   ...
   *
   * @returns {{
   *   type: object,           매칭된 유형 객체
   *   distance: number,       1단계 맨해튼 거리
   *   exactDimensions: number, 2단계 정확 일치 차원 수
   *   rawScoreError: number,  3단계 원점수 오차 합
   *   reachedStage: 1|2|3|'random'
   * }}
   */
  /**
   * 유사도 % 계산 (압축 매핑 78~99%)
  ...
  function matchType(userResult) {
    var types = window.TYPES_SPENDING_TYPE;
    if (!types || !types.length) {
      console.error('[scoring] TYPES_SPENDING_TYPE 이 로드되지 않았습니다.');
      return null;
    }
    ...
    // ── 동점 → random ────────────────────────────────────────────
    var winnerRandom = candidates3[Math.floor(Math.random() * candidates3.length)];
    return {
      type:            winnerRandom,
      distance:        minDist,
      matchPercent:    calcMatchPercent(minDist),
